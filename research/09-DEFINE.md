...
| R7 | Identify authentication algorithm | **RE-SCOPE, corrected by EXP-11 (2026-09-13)** — ESP/IKE integrity algorithm: **O at T0/T1** (`ike_integ`, built). Peer auth method (PSK/cert/EAP): the original plan ("I at T1 via CERTREQ/SIGHASH presence") tested empirically before shipping and found NOT reliably per-tunnel-observable — SIGNATURE_HASH_ALGORITHMS is emitted unconditionally, and CERTREQ reflects the *responder's* fleet-wide trust-anchor policy (sent before `IDi` identifies which connection applies), not this SA's actual method, confirmed by a same-responder PSK-vs-cert differential test. **NOT-OBSERVABLE at T0/T1** for the per-tunnel method (same encryption-boundary pattern as mode, EXP-08); `responder_cert_capability` kept as its own, correctly-scoped fleet-level signal | A8, EXP-11 |
...
| R9 | Identify SA characteristics | **RE-SCOPE** — split into O items (SPI, proposals) and **measured** items (lifecycle, rekey cadence). SPI + proposals: **O at T1, built** (`ike_spi`, `ike_crypto`, added 2026-09-13 — SPI was tracked internally but never exposed as a citable finding until the full-project audit caught it). Lifecycle/rekey-cadence measurement: **not built** — genuinely deferred, needs a longer-running multi-rekey testbed arm, stated as open in `TODO.md` rather than silently dropped | A11, CS-03 |
...
| R16 | Executive + technical reports, risk score, threat matrix, AI confidence score | **ACCEPT, RE-SCOPED — delivered differently than first planned, corrected here 2026-09-13.** Reports + a per-baseline, cited, sensitivity-tested score exist (`tunnelscope/report/`, `score/`); a literal "threat matrix" artifact does not — the verdict tables (per-baseline, per-severity) serve that role. Confidence is **not** conformal: once 8 of 9 shipped capabilities turned out fully deterministic (EXP-01/03/04/06/07/08), there was no ML classification needing a calibrated confidence set to build. The one ML component (leakage, CS-01) reports a Bayes-error/MI estimate instead, which is a more principled uncertainty measure for that specific instrument than a generic conformal wrapper would have been. Recorded honestly: conformal prediction was the *planned* mechanism, not the *delivered* one | DL-03, §4.4; correction: `tunnelscope/score/score.py`, `tunnelscope/leakage/leakage.py` |
