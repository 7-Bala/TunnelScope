...
#
# Usage: run_encap.sh <arm-name> <peer-address>
# Output: testbed/captures/encap/<arm>.pcap + <arm>.groundtruth.json
set -euo pipefail

ARM="${1:?usage: run_encap.sh <arm> <peer-address>}"
PEER="${2:?usage: run_encap.sh <arm> <peer-address>}"
HERE="$(cd "$(dirname "${BASH_SOURCE[0]}")" && pwd)"
OUT="$HERE/../captures/encap"
mkdir -p "$OUT"
PCAP="/captures/encap/${ARM}.pcap"
...
sleep 1
docker exec sih26e-alice swanctl --initiate --child "$ARM" 2>&1 | tail -2
