...
**Quality and process**
- `pytest` (176 tests), GitHub Actions CI, a ground-truth check against each endpoint's own `swanctl` output, a dataset hash check, and a check against other people's public captures.
...
- **Would you use a local AI model, like an LLM?** Every model that decides something — traffic type, mode, anomaly, mixed traffic — stays ours, trained on our own captured data; that doesn't change. What we're designing (`build/10-LOCAL-AI-RUNTIME.md`) is a small, optional, fully on-device model via Apple's MLX, used only to reword text we've already computed into more natural sentences — never to invent a finding. It runs entirely on the presenter's Mac, no network call, off by default, and every rephrased panel still shows it came from a fact, never from the model.
