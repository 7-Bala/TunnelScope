...
| Model | **`openbmb/MiniCPM5-2B-MLX`** — the publisher's OWN official MLX release (confirmed to exist 2026-09-22), preferred over third-party re-conversions (`mlx-community/MiniCPM5-2B-8bit`, `abenzerps/MiniCPM5-2B-MLX-4bit`) for supply-chain trust. Second choice: `unsloth/gemma-4-E4B-it-UD-MLX-4bit`, the only model compared that was actually benchmarked on an M4 — by far the fastest prompt-processing of the set (627–637 tok/s vs 165–168 for the 9B models). Superseded: `Qwen3.5-4B-MLX-4bit` (see below) | MiniCPM5-2B (OpenBMB, released 2026-09-07) scores 53.9 on OpenBMB's own 34-benchmark suite vs Qwen3.5-4B's 51.1 — beating a 4B model at half the size — and ships Apache 2.0 (cleanly resolves the licence guardrail). Its instruction-following score specifically is reported as "closer to parity" with bigger models, not a clear win — the honest caveat, since that's the exact metric this task depends on. Smallest footprint of anything considered (~1.1–2.5GB depending on quant) — most headroom on the M4 alongside the dashboard/engine/Docker |
